...
One of distinctive feature of greed is that you may mix several model together. Using such an approach one may perform a clustering of several view of the same individuals with different generative models. In all case this approach suppose that the different views are generated in an independent way when the clustering assignment is known. To explore this capability we will begin with the Fifa dataset and cluster simultaneously the categorical features with an `LcaPrior()` and the continuous one with a `GmmPrior()`.


### Lca + Gmm, the Fifa data-set 

We first load the `?Fifa` dataset (several features of Soccer players in the Fifa 2020 videogame). We remove the first two columns that give the players names and nationality, and we kept only the players which earn more than  3000000 euros.
...
# create the MixedModels, as expected we will use an LcaPrior for the factors 
...
ggplot(clust_positions)+ background_image(Fifa_positions$bg_img)+
