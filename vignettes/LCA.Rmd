---
title: "Categorical data clustering with the Latent Class Analysis"
output: 
  rmarkdown::html_vignette:
    ...
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{Categorical data clustering with the Latent Class Analysis}
...
```
```{r}
kable(table(Edibility=label, Cluster=clustering(sol_mush)), format='html', row.names = T)
```

The hybrid genetic algorithm found a solution with $K=`r sol_mush@K`$ clusters which is quite over-segmented while displaying a good separation among edible and poisonous mushrooms. Partition's NMI is `r round(NMI(label, clustering(sol_mush)),2)` which is explained by the over-segmentation of the solution compared to the $2$-class problem.
...
```{r cut-K2-mushroom}
sol2 = cut(sol_mush, 2)
knitr::kable(table(Edibility=label, Cluster=clustering(sol2)), format='html', row.names = T)
```
...
```{r survey-lca-plot2,fig.show='hold',out.width="100%",fig.dim=c(10,8)}
plot(sol,type='marginals')
```


The clusters appear to be quite different and coherent. For example, the seveth group (second top row, in purple) display a strong tendency to like Opera and Classical music and dislike more *modern* genres (Techno, Trance, Punk or Hi-Hop), whereas the 8th group (top row, in pink) corresponds to music lovers with quite eclectic musical taste across the proposed survey's genres.

> In addition, the conditional maximum a posteriori estimates of $\theta{kj}$ are qvailable through the `coef()` method. 

```{r}
params = coef(sol)
```

```{r thetask, include=TRUE, echo=FALSE, results='asis'}
vars = c("Country","Latino")
for (var in vars) {
  print(kable(params$thetas[[var]], format='html', row.names='true', caption = var))
  cat('\n\n\n')
}
# kables(sapply(names(thetas),
#               function(ind){
#                 kable(thetas[[ind]], format='html', row.names='true', caption = ind)
#               }),
#        format = 'html')
# sapply(names(thetas),
#               function(ind){
