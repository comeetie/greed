...
sol=greed(X)
```
The algorithm found $K=`r K(sol)`$ clusters which are quite balanced. To explore the results, we plot the dendogram found and used the marginals plot to depict the conditional probilities of the different responses knowing the clusters assignment.
...
params = coef(sol)
means_scores = lapply(params$Thetak,\(x){apply(x[,1:5],1,\(r){sum(r*1:5)})})
means_scores_long = do.call(rbind,
                            map2(means_scores,names(means_scores),\(x,y){
...
The first cluster is close to the global mean, the second corresponds to Classical music and Opera lovers, the third to peoples that like almost all the musical genres, the forth to Rockers, the Fifth to Hard Rockers and the sixth to Rap lovers.
