...
Graph data arise in various scientific fields from biology to sociology, and accounts for relationship between objects. These objects are expressed as __nodes__, while a relationship between two objects is expressed as an __edge__. Hence, graph data may be expressed and stored in an __adjacency matrix__ $\mathbf{X} = \{ x_{ij} \}$ where $x_{ij}=1$ means that objects $i$ and $j$ are connected. Weighted versions are also possible.  
...
\theta_{k,l} $ \sim \textrm{Beta}(a0, b0), \\
 ...
 * $alpha$ whis is set to $1$ by default.
...
```{r plot-connectivity-matrix, include=TRUE, echo=FALSE}
