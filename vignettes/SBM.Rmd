...
```{r, fig.show='hold', echo=FALSE, include=TRUE}
# Regular SBM
sol_sbm_k3 = cut(sol_sbm,3)
confmat_sbm = knitr::kable(table(clustering(sol_sbm_k3), Books$label), format='html', caption = "Regular SBM", align="l")
# DcSbm
confmat_dcsbm = knitr::kable(table(clustering(sol_dcsbm), Books$label), format='html', caption = "DC-SBM")
knitr::kables(list(confmat_sbm, confmat_dcsbm), format='html', caption = "Confusion matrices")
```

# Categorical edges and multidimensional graphs
...
To each modality (or slice), we can associate a binary graph corresponding the interaction, and plot its he aggregated adjacency matrix. The multiple views allow to interpret the clusters in term of the modalities. In this example, we have three *slices* corresponding to the three modalities and :
 ...
 2. The second slice (*friendship*) is consistent with the first slice, since the two groups have a pronounced community structure, with group 1 being densely connected. There are no friendships links between the two groups.
 3. The third slice (*no relation*) highlight that individuals in group 2 mostly share no relation.
 
To sum up, the algorithm found 2 groups/clusters. The first is composed of a small, densely connected community of friend tribes, whereas the second is larger and more heterogeneous with mostly no interactions between the tribes. The two clusters either have enmity relations or no relation at all.

```{r, fig.show='hold',out.width="100%",fig.width=8,fig.height=5.5}
plot(sol_newguinea,type='blocks')
```

To further the analysis, it is possible to visualize the two graphs of enmity and friendships on top of each other. The node shape symbolizes the cluster, while the color of the edge characterizes the modality. One can see that the first cluster makes a 4-node clique for friendship while having a strongly pronounced bi-partite structure for enmity, hence comforting the previous analysis. 
```{r, fig.show='hold',out.width="70%",fig.width=8,fig.height=5.5}
...
As explained above, some graphs may encode different kinds of relationships. This general case of multidimensional network correspond to a 3-dimensional encoding $V$ different graphs. Note that graphs with multinomial edges are a particular case where an edge $(i,k)$ can only belong to one of the views.

In the perspective of graphs clustering, we search for **one** common partition of the nodes among the different graphs. From the statistical point-of-view of the DLVMs framework, it amounts to stack observational models (*i.e.* SBM, dc-SBM, or MultinomialSBM) on each of the graphs, that are supposed independent conditionally on the unknown partitions :
```{=tex}
\begin{equation}
...
**Practicalities:** The `MixedModel` class allows any observational model, however they need to be initiated via the following syntax: `<your_desired_model>Prior()`. Without going into technical details, the distinction is necessary to account for the shared aspect of the partition $Z$ between all models.
