...
params = coef(sol)
means_scores = lapply(params$Thetak,\(x){apply(x,1,\(r){sum(r*1:5)})})
means_scores_long = do.call(rbind,map2(means_scores,names(means_scores),\(x,y){tibble(cluster=1:K(sol),mean=x,var=y)}))  %>% mutate(var = gsub("\\."," ",var))
