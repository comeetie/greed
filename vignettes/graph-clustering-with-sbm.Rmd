...
```{r, fig.show='hold',out.width="70%",fig.width=8,fig.height=5.5}
soldc = fit(sbm$x,25,new("dcsbm"))
plot(soldc,type='blocks')
```

Eventualy you may change the algorithm genetic (default), greed multiple start of simple greedy and km greed initialized with spectral clustering. See the doc for details on the algorithm parameters. 


One example with the polbooks dataset:
```{r, fig.show='hold',out.width="70%",fig.width=8,fig.height=5.5}
data("Xpolbooks")
solpol=fit(Xpolbooks,10,new("dcsbm"),new("hybrid",pop_size=30))
...
```{r, fig.show='hold',out.width="70%",fig.width=8,fig.height=5.5}
data("Xpolblogs")
solblogs=fit(Xpolblogs,25,new("dcsbm"),new("seed"))
...
A better fit with a genetic algorithm ?
```{r, fig.show='hold',out.width="70%",fig.width=8,fig.height=5.5}
solblogshyb =fit(Xpolblogs,25,new("dcsbm"),new("hybrid",pop_size=40))
plot(solblogshyb,type='path')
table(Lpolblogs,cut(solblogs,2)@cl)
solblogshyb@icl-solblogs@icl
```