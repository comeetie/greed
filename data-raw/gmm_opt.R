...
timming <- microbenchmark(
  {
    greed:::fit_greed(model, data, cli, type = "swap", nb_max_pass = 1)
  },
  times = 2,
  unit = "s"
)
...
Nk <- 400
X <- do.call(rbind, lapply(1:nrow(xy), \(i){
  MASS::mvrnorm(Nk, xy[i, ], runif(1) * 0.3 * diag(2))
}))
plan(multisession)
...
centers <- do.call(rbind, lapply(sol@obs_stats[[2]], \(x){
  x$m
}))
ggplot(data.frame(x = X[, 1], y = X[, 2])) +
  geom_point(aes(x = x, y = y, color = factor(sol@cl)), alpha = 0.3) +
