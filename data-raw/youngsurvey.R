...
selected <- Young_people_survey %>%
  ...
  mutate(sel = if_else(string <= 10, TRUE, FALSE)) %>%
  pull(sel)
...
means_scores <- lapply(params$Thetak, \(x){
  apply(x, 1, \(r){
    sum(r * as.numeric(names(r)))
  })
})
means_scores_long <- do.call(rbind, purrr::map2(means_scores, names(means_scores), \(x, y){
  tibble(cluster = names(x), mean = x, var = y)
})) %>% mutate(var = gsub("\\.", " ", var))
means_scores_glob <- interests %>%
  summarise_all(\(x){
    mean(x, na.rm = TRUE)
  }) %>%
...
cluster_profile <- function(clid) {
  df <- gg %>%
    filter(cluster == paste0("cluster", clid)) %>%
    filter(abs(dm) > 0.3)
