...
SpDcSbm::SpDcSbm(const arma::sp_mat& xp,int Ki,double alphai,arma::vec& clt,bool verb){
  alpha = alphai;
  x  = xp;
  xt = xp.t();
  N  = x.n_rows;
...
double SpDcSbm::icl_emiss(const List & obs_stats){
  ...
  arma::sp_mat edges_counts =as<arma::sp_mat>(obs_stats["x_counts"]);
...
double SpDcSbm::icl_emiss(const List & obs_stats,int oldcl,int newcl){
  arma::vec counts =as<arma::vec>(obs_stats["counts"]);
  arma::vec din =as<arma::vec>(obs_stats["din"]);
  arma::vec dout =as<arma::vec>(obs_stats["dout"]);
  arma::sp_mat edges_counts =as<arma::sp_mat>(obs_stats["x_counts"]);
  ...
  int l = 0;
  int cc = 0;
  for (arma::sp_mat::iterator  i = edges_counts.begin(); i != edges_counts.end(); ++i){
    k=i.row();
    l=i.col();
    if(counts(k)*counts(l)!=0){
      cc = counts(k)*counts(l);
      // lets go
      // Rcout << edges_counts(k,l) << "-----" << icl_emiss << std::endl;
...
arma::mat SpDcSbm::delta_swap(int i,arma::uvec iclust){
  ...
  for(int j = 0; j < iclust.n_elem; ++j) {
    ...
    if(k!=oldcl){
      ...
      arma::sp_mat delta_ec = sp_cross(col_sum,row_sum.t(),self,oldcl,k,K);
      arma::sp_mat new_ec = add_spmatpat(x_counts,delta_ec);
      arma::sp_mat old_ec = which_spmatpat(x_counts,delta_ec);
      ...
      List old_stats = List::create(Named("counts", counts),Named("din", din),Named("dout", dout), Named("x_counts", old_ec));
      ...
      Rcout << delta(k) << std::endl;
      old_stats = List::create(Named("counts", counts),Named("din", din),Named("dout", dout), Named("x_counts", x_counts));
      new_ec=x_counts+delta_ec;
      new_stats = List::create(Named("counts", new_counts),Named("din", new_din),Named("dout", new_dout), Named("x_counts",new_ec));
      double de=icl(new_stats)-icl(old_stats);
      Rcout << de << std::endl;
      Rcout << "---" << std::endl;
    }
  }
...
void SpDcSbm::swap_update(const int i,const int newcl){
  int self=x(i,i);
  int oldcl = cl(i);
  arma::sp_mat col_sum = gsum_col(cl,x,i,K);
  col_sum(oldcl)=col_sum(oldcl)-self;
...
double SpDcSbm::delta_merge(int k, int l){
  ...
  new_counts(k)=0;
  arma::sp_mat delta_ec = sp_cross(x_counts.col(k),x_counts.row(k),0,k,l,K);
  arma::sp_mat new_ec = add_spmatpat(x_counts,delta_ec);
  arma::sp_mat old_ec = which_spmatpat(x_counts,delta_ec);
...
double SpDcSbm::delta_merge_correction(int k,int l,int obk,int obl,const List & old_stats){
  ...
  arma::sp_mat old_x_counts =as<arma::sp_mat>(old_stats["x_counts"]);
  cc = counts(k)*counts(l);
  arma::uvec kl;
  kl << k << l << arma::endr;
  arma::uvec mkl;
...
void SpDcSbm::merge_update(int k,int l){
