...
MarSbm::MarSbm(arma::sp_mat& xp,arma::sp_mat& xpobs,double alphai,double a0i,double b0i,double a0obsi,double b0obsi,arma::vec& clt,bool verb) : MissSbm(xp,xpobs,alphai,a0i,b0i,a0obsi,b0obsi,clt,verb)
{
...
double MarSbm::icl_emiss(const List & obs_stats){
...
double MarSbm::icl_emiss(const List & obs_stats,int oldcl,int newcl){
  arma::vec counts =as<arma::vec>(obs_stats["counts"]);
  arma::mat edges_counts =as<arma::mat>(obs_stats["x_counts"]);
  arma::mat obs_counts =as<arma::mat>(obs_stats["x_counts_obs"]);
...
double MarSbm::delta_merge_correction(int k,int l,int obk,int obl,const List & old_stats){
