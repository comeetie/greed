...
double NmarBdSbmUndirected::icl_emiss(const List & obs_stats){
  ...
  d_counts = (matcount.diag()-counts)/2;
  ec = obs_counts.diag()/2;
  cmatobs.diag() =  lgamma(a0obs+ec)+lgamma(d_counts-ec+b0obs)+lgamma(a0obs+b0obs) - lgamma(a0obs) - lgamma(b0obs) - lgamma(d_counts+a0obs+b0obs);
  arma::uvec lonely = arma::find(counts==1);
  for (arma::uword i = 0;i<lonely.n_elem;++i){
    cmat(lonely(i),lonely(i))=0;
    cmatobs(lonely(i),lonely(i))=0;
  }
  double icl_emiss=accu(arma::trimatl(cmat+cmatobs));

  return icl_emiss;
}

double NmarBdSbmUndirected::icl_emiss(const List & obs_stats,int oldcl,int newcl){
  arma::vec counts =as<arma::vec>(obs_stats["counts"]);
  arma::mat edges_counts =as<arma::mat>(obs_stats["x_counts"]);
  arma::mat obs_counts =as<arma::mat>(obs_stats["x_counts_obs"]);
  ...
  for (arma::uword i = 0;i<si.n_rows;++i){
    ...
    if(counts(k)*counts(l)!=0){
      ...
      if((k==l) && (counts(k)>1)){
        ...
        icl_emiss += lgamma(a0obs+obs_counts(k,l)/2)+lgamma(b0obs+cc-obs_counts(k,l)/2)+lgamma(a0obs+b0obs)-lgamma(a0obs)-lgamma(b0obs)-lgamma(a0obs+b0obs+cc);
...
double NmarBdSbmUndirected::delta_merge_correction(int k,int l,int obk,int obl,const List & old_stats){
