...
setMethod(f = "plot", 
          ...
          definition = function(x){
            K = length(x@counts)
            D = dim(x@x_counts)[2]
            gg=data.frame(kc=rep(cumsum(x@counts),D),
                          lc=rep(1:D,each=K),
                          sizek = rep(x@counts,D),
                          sizel = rep(1,K*D), 
                          count=as.vector(x@x_counts))
            ggplot2::ggplot(gg)+ggplot2::geom_tile(ggplot2::aes(y=kc-sizek/2,x=lc-sizel/2,height=sizek,width=sizel,fill=count/sizek,alpha=count/sizek))+
              ...
              ggplot2::scale_y_continuous("Clusters",breaks=cumsum(x@counts),labels = paste0(round(100*x@counts/sum(x@counts)),"%"),minor_breaks = NULL)+
              ggplot2::theme_bw()         
          });
